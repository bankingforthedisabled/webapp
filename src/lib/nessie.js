...
export const getCustomers = () => {
  return axios.get(`${base}/customers?key=${api_key}`)
    .then(res => {
      return res;
    })
...
export const getCustomerData = () => {
  return axios.get(`${base}/accounts/${account_number}/customer?key=${api_key}`)
    .then(res => {
...
export const getCustomerLoans = (customerId) => {
  return axios.get(`${base}/accounts/${customerId}/loans?key=${api_key}`)
    .then(res => {
      return res.data
    })
    .catch(err => console.log(err));
